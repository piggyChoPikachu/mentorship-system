...
def published_profile_page():
    """Show published profile for current user or another user by person_id"""
    viewer_id = session.get("user_id")
    person_id = request.args.get("person_id", type=int) or viewer_id
    ...
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                person = {
                    ...
                    "address": row[6],
                    "email": row[7],
                    "profile_published": row[8],
                    "preferences_published": row[9]
                }
                ...
                if preferences_published:
                    ...
                    preferences_data = [
                        {
                            "topic_id": pref_row[0],
                            "topic_name": pref_row[1],
                            "preference_role": pref_row[2]
                        }
                        for pref_row in cur.fetchall()
                    ]
        ...
        return render_template(
            ...
            is_own_profile=(person_id == viewer_id)
        )
...
# ============================================================
# MENTORSHIP MANAGEMENT — new routes to ADD to app.py
# ============================================================
# Place these routes right after your existing
# @app.get("/api/mentorship-management/active") route.
# ============================================================


# ── 1. Overview: active + past  (includes unread count) ────

@app.get("/api/mentorship-management/overview")
@login_required
def api_mentorship_overview():
    """Return active and past mentorships, plus unread message count per mentorship."""
    user_id = session.get("user_id")
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                # current user info
                cur.execute(
                    "SELECT id, username, first_name, last_name, identity_role FROM person WHERE id = %s",
                    (user_id,)
                )
                me = cur.fetchone()
                if not me:
                ...
                current_user = {
                    "id": me[0], "username": me[1],
                    "first_name": me[2], "last_name": me[3], "identity_role": me[4]
                }

                # unread counts: messages sent by the OTHER person that I have not read yet
                cur.execute(
                    """
                    SELECT mentorship_id, COUNT(*) AS unread_count
                    FROM mentorship_message
                    WHERE is_read    = FALSE
                      AND sender_id != %s
                      AND mentorship_id IN (
                          SELECT id FROM mentorship
                          WHERE student_id = %s OR alumni_id = %s
                      )
                ...
                def fetch_list(status):
                    cur.execute(MENTORSHIP_QUERY, (user_id, user_id, user_id, status))
                    rows = []
                    for row in cur.fetchall():
                        mid = row[0]
                        rows.append({
                            ...
                            "end_reason":          row[8],
                            "topic_name":          row[9],
                            "other_person_id":     row[10],
                            "other_first_name":    row[11],
                            "other_last_name":     row[12],
                            "other_identity_role": row[13],
                            "other_email":         row[14],
                            "unread_count":        unread_map.get(mid, 0),  # ← new
                        })
                    return rows

                active_list    = fetch_list("active")
                completed_list = fetch_list("completed")
                cancelled_list = fetch_list("cancelled")

        return jsonify({
            "ok":             True,
            "current_user":   current_user,
            "mentorships":    active_list,
            "past_mentorships": completed_list + cancelled_list,
        }), 200

    except Exception as e:
        print(f"Error in mentorship overview: {e}")
        return jsonify({"error": str(e)}), 500


# ── 2. Update status: complete or cancel ───────────────────

@app.post("/api/mentorship-management/<int:mentorship_id>/status")
@login_required
def api_update_mentorship_status(mentorship_id):
    """
    Either user in the pair can complete or cancel.
    Body: { "status": "completed"|"cancelled", "reason": "optional text" }
    """
    user_id    = session.get("user_id")
    data       = request.get_json(silent=True) or {}
    new_status = (data.get("status") or "").strip().lower()
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    SELECT id FROM mentorship
                    WHERE id = %s
                      AND (student_id = %s OR alumni_id = %s)
                      AND status = 'active'
                    """,
                    (mentorship_id, user_id, user_id)
                )
                if not cur.fetchone():
                    return jsonify({"error": "Mentorship not found or not active"}), 404
                ...
                cur.execute(
                    ...
                    WHERE id = %s
                    """,
                    (new_status, reason, mentorship_id)
                )
            conn.commit()

        label = "completed" if new_status == "completed" else "cancelled"
        return jsonify({"ok": True, "message": f"Mentorship marked as {label}."}), 200

    except Exception as e:
        print(f"Error updating mentorship status: {e}")
        return jsonify({"error": str(e)}), 500


# ── 3. Messages: GET (load thread + mark as read) ──────────

@app.get("/api/mentorship-management/<int:mentorship_id>/messages")
@login_required
def api_get_messages(mentorship_id):
    """
    Load all messages for this mentorship thread.
    At the same time, mark all messages sent by the OTHER person as read
    — because opening the modal means the current user has now seen them.
    """
    user_id = session.get("user_id")
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                # security: user must be in this mentorship
                cur.execute(
                    "SELECT id FROM mentorship WHERE id = %s AND (student_id = %s OR alumni_id = %s)",
                    (mentorship_id, user_id, user_id)
                )
                if not cur.fetchone():
                ...
                cur.execute(
                    """
                    UPDATE mentorship_message
                    SET is_read = TRUE
                    WHERE mentorship_id = %s
                      AND sender_id    != %s
                      AND is_read       = FALSE
                    """,
                    (mentorship_id, user_id)
                )
                ...
                cur.execute(
                    ...
                    SELECT
                        mm.id,
                        mm.sender_id,
                        COALESCE(NULLIF(p.first_name,''), p.username) AS sender_name,
                        mm.body,
                        mm.sent_at
                    FROM mentorship_message mm
                    JOIN person p ON p.id = mm.sender_id
                    WHERE mm.mentorship_id = %s
            ...
            conn.commit()  # commit the mark-as-read update
...
# ── 4. Messages: POST (send a message) ─────────────────────

@app.post("/api/mentorship-management/<int:mentorship_id>/messages")
@login_required
def api_send_message(mentorship_id):
    """Send a message. is_read starts as FALSE — receiver hasn't seen it yet."""
    user_id = session.get("user_id")
    data    = request.get_json(silent=True) or {}
    ...
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    """
                    INSERT INTO mentorship_message (mentorship_id, sender_id, body, is_read)
                    VALUES (%s, %s, %s, FALSE)
                    RETURNING id
                    """,
                    (mentorship_id, user_id, body)
                )
                new_id = cur.fetchone()[0]
            conn.commit()

        return jsonify({"ok": True, "message_id": new_id}), 201
